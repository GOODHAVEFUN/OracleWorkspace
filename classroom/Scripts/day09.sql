...
SELECT M.FIRST_NAME ||' ' ||M.LAST_NAME ,COUNT(E.MANAGER_ID) 
FROM EMPLOYEES e 
JOIN EMPLOYEES M
...
SELECT COUNT(*) FROM PLAYER p ; --전체 행수

SELECT AVG(HEIGHT) FROM PLAYER p ; --업데이트전 평균

SELECT COUNT(*) --COUNT(HEIGHT)는 0으로 조회되는데 COUNT는 NULL값을 자동으로 제외시키기 때문이다
FROM PLAYER p 
WHERE HEIGHT IS NULL; --NULL값인 행수

SELECT HEIGHT 
FROM PLAYER p 
WHERE HEIGHT IS  NULL; --확인

SELECT COUNT(HEIGHT)
FROM PLAYER p 
WHERE HEIGHT IS NOT  NULL; --NULL값이 아닌 행수 

SELECT HEIGHT 
FROM PLAYER p 
WHERE HEIGHT IS NOT NULL; --확인

UPDATE  PLAYER 
SET HEIGHT = 170
WHERE HEIGHT IS NULL; --업데이트

SELECT AVG(HEIGHT) FROM PLAYER p ; --평균값

--* PLAYER 테이블에서 팀 별 최대 몸무게 조회
SELECT T.TEAM_NAME ,MAX(P.WEIGHT) 
FROM PLAYER p 
JOIN TEAM t ON P.TEAM_ID = T.TEAM_ID 
GROUP BY T.TEAM_NAME  ;

--* AVG 함수 쓰지 않고 PLAYER 테이블에서 선수들의 평균 키 구하기(NULL 미포함)
SELECT (SUM(HEIGHT) / COUNT(HEIGHT)) 평균 FROM PLAYER p ; 
--COUNT(HEIGHT)는 NULL값이 들어간 행은 제외된다. 
--만약 NULL값을 포함하고 싶다면 COUNT(*)을 넣어주면된다

--* PLAYER 테이블에서 팀 별 최대 몸무게인 선수 전체 정보 검색
SELECT TEAM_ID ,MAX(WEIGHT)  FROM PLAYER p GROUP BY TEAM_ID;
--팀별 최대 몸무게
SELECT T.TEAM_NAME ,P.*
FROM PLAYER p 
JOIN TEAM t 
ON P.TEAM_ID =T.TEAM_ID 
WHERE (P.TEAM_ID,P.WEIGHT ) IN (SELECT TEAM_ID ,MAX(WEIGHT)  FROM PLAYER p GROUP BY TEAM_ID)
...
SELECT TO_CHAR(1234321, '000,999,999')
FROM DUAL;
SELECT TO_CHAR(1234321, '999,999,999')
FROM DUAL;
...
-- SELF JOIN 해야함
SELECT E.FIRST_NAME || ' ' || E.LAST_NAME 사원이름  , E2.FIRST_NAME || ' ' || E2.LAST_NAME 매니저이름
FROM EMPLOYEES e
JOIN EMPLOYEES e2 
